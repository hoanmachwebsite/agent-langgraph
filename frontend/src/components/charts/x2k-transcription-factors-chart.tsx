...
import { transformTranscriptionFactorsData } from "@/lib/utils";
import {
...
const TooltipBarChart: React.FC<TooltipBarChartProps> = ({
  isVisible,
  position,
  transcriptionFactor,
  pValue,
  ...
  return (
    <div
      className="tooltip-bar-chart border-border dynamic-bg-light fixed z-50 max-w-[280px] rounded-md border p-3 shadow-lg"
      style={{
        ...tooltipStyle,
    ...
    >
      <div className="">
        <div className="text-muted-foreground text-sm">
          <span className="">Transcription Factor: </span>
          <span className="">{transcriptionFactor}</span>
        </div>
        <div className="text-muted-foreground text-sm">
        ...
        <div className="text-muted-foreground text-sm">
          <span className="">Enriched targets: </span>
...
interface X2KTranscriptionFactorsChartProps {
  data: any;
...
export const X2KTranscriptionFactorsChart: React.FC<
  X2KTranscriptionFactorsChartProps
> = ({ data, chartType, refChart, isLoading = false, isZoomed = false }) => {
  ...
  const { bargraphData, tableData } = transformTranscriptionFactorsData(data);
  ...
  const testBargraphData = useMemo(() => {
    ...
    return Array.from({ length: 25 }, (_, index) => ({
      transcriptionFactor: `TF_${index + 1}`,
      pValue: Math.random() * 0.1,
      minusLog10PValue: Math.random() * 10 + 1,
      targetCount: Math.floor(Math.random() * 50) + 1,
      enrichedTargets: `Target genes for TF_${index + 1}`,
    }));
  }, [bargraphData]);
  ...
  useEffect(() => {
    ...
    g.selectAll(".bar")
      ...
      .attr("fill", (d) => {
        ...
        return pValue < X2K_SIGNIFICANCE_THRESHOLD
          ? X2K_TF_BAR_COLORS.SIGNIFICANT
          : X2K_TF_BAR_COLORS.NOT_SIGNIFICANT;
      })
      .attr("stroke", "none")
      ...
      .on("mouseover", function (event: any, d: any) {
        ...
        setTooltip({
          isVisible: true,
          position,
          transcriptionFactor: d.transcriptionFactor || "Unknown",
          pValue: d.pValue || 0,
    ...
    // Add transcription factor labels on the left
    g.selectAll(".tf-label")
      .data(currentPageData)
      .enter()
      .append("text")
      .attr("class", "tf-label")
      ...
      .text((d) => d.transcriptionFactor || "Unknown")
      .on("mouseover", function () {
        // Hide tooltip when hovering over transcription factor labels
        if (tooltipTimeoutRef.current) {
          window.clearTimeout(tooltipTimeoutRef.current);
        }
        setTooltip({
    ...
    return (
      <div className="flex h-[400px] items-center justify-center">
        <Spinner
          ...
          label="Loading transcription factors data..."
        />
      </div>
    );
  }
    ...
    return (
      <div className="flex h-[384px] items-center justify-center">
        <p className="text-gray-500">
          No transcription factors data available to display.
        </p>
      </div>
    );
  }
            ...
            <div className="flex items-center">
              <svg width="240" height="20">
                <rect
                  ...
                  rx="2"
                  fill={X2K_TF_BAR_COLORS.SIGNIFICANT}
                />
                <text
            ...
            <div className="flex items-center">
              <svg width="180" height="20">
                <rect
                  ...
                  height={X2K_LEGEND_BOX_SIZE}
                  rx="2"
                  fill={X2K_TF_BAR_COLORS.NOT_SIGNIFICANT}
                />
                <text
                  x={X2K_LEGEND_BOX_SIZE + 8}
                  y="10"
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="">
                ...
                {/* {currentPageData.length > 0 && (
                  <>
                    {(currentPage - 1) * X2K_BARGRAPH_ITEMS_PER_PAGE + 1} to{' '}
                    {Math.min(
                      currentPage * X2K_BARGRAPH_ITEMS_PER_PAGE,
                      testBargraphData.length
                    )}{' '}
                    of {testBargraphData.length} items
                  </>
                )} */}
              </div>
            </div>
